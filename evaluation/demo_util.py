...
from training.video_encoder import CAVP_Inference
...
def reencode_video_with_diff_fps(video_path: str, tmp_path: str, extraction_fps: int, start_second, truncate_second) -> str:
    ...
    if truncate_second is None:
        ...
        cmd += f'-y -i {video_path} -an -filter:v fps=fps={extraction_fps} {new_path}'  # -an 表示不包含音频
...
class Extract_CAVP_Features(torch.nn.Module):
    ...
    def __init__(self, method="Ours", ckpt_path=None, device=None, diff_config_path=None, tmp_path="./temp_folder"):
        super(Extract_CAVP_Features, self).__init__()
        self.fps = 4
        self.batch_size = 40
        self.video_shape = (224,224)

        self.device = device
        self.tmp_path = tmp_path

        print("|| Extract_CAVP_Features method: ", method)
        if method == "Ours":
            print("Loading Our Model from: {}".format(ckpt_path))
            assert ckpt_path is not None
            self.stage1_model = CAVP_Inference(video_encode='Slowonly_pool', embed_dim=512, ckpt_path=ckpt_path).to(device)
        elif method == "Diff-Foley":
            print("Initalize Diff-Foley Stage1 CAVP Model")
            config = OmegaConf.load(diff_config_path)
        ...
        self.img_transform = transforms.Compose([
            transforms.Resize(self.video_shape),
            transforms.ToTensor(),
        ])
    ...
    @torch.no_grad()
    def forward(self, video_path, start_second=None, truncate_second=None, test="eval"):
        ...
        if video_feats:
            ...
            # print(f"need at least one array to concatenate with {video_path_low_fps}")
            video_contrastive_feats = None
        if test == "eval":
